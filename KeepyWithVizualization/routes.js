...
const router = (app) => {
  ...
  app.post("/messages", async (request, response) => {
    ...
    if (process.env.BUFFER_ENABLED === "FALSE") {
      try {
        ...
        pool.query("INSERT INTO messages SET ?", data, (error, result) => {
          ...
          return response
            .status(201)
            .send(`Message added with ID: ${result.insertId}`)
            .end();
        });
      } catch (error) {
    ...
    pool.query("INSERT INTO messages SET ?", data, (error, insertResult) => {
      ...
      pool.query(
        "SELECT COUNT(*) AS channelIdCount FROM messages WHERE channelId IS NULL",
        (error, result) => {
          if (error) {
          ...
          if (countChannelId < process.env.BUFFER_SIZE) {
            return response
              .status(201)
              .send(`Message added with ID: ${insertResult.insertId}`);
          }
            ...
            async (error, result) => {
              ...
              try {
                const gatewayResponse = await fetch(
                  process.env.GATEWAY_BUNDLE_URL,
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                ...
                if (gatewayResponse.status === 400) {
                  return response
                    .status(500)
                    .send(
                      `Error: Gateway response: ${gatewayResponse.statusText}`
                    );
                }
                ...
                if (channelId) {
                  result.forEach((item) => {
                    pool.query(
                      `UPDATE messages SET channelId = '${channelId}' WHERE ID = ${item.id}`,
                      (error, result) => {
                        if (error) {
                          return response
                            .status(500)
                            .send("Error performing UPDATE");
                        }
                      }
                    );
                  });
                  ...
                  response
                    .status(201)
                    .send(`All messages added with Channel ID: ${channelId}`);
                }
              } catch (error) {
